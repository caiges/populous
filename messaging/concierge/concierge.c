/*
 *	Concierge - IM client for Populous
 *
...
static void add_account(char username[], char password[], char protocol_id[]) {
...
static void print_protocols(){
    /* List the available protocols and exit*/
    GList *iter;
    GList *names = NULL;
    int i;
    ...
    for (i = 0; iter; iter = iter->next) {
        ...
        if (info && info->name) {
            printf("{'id': '''%s''','name': '''%s''','description': '''%s''','summary': '''%s'''}\n",
                   info->id, info->name, info->description, info->summary);
...
int main(int argc, char *argv[]) {	
    ...
    /* Redirect errors to err file */
    freopen ("concierge.err", "w", stderr);
    ...
    add_account("kerckhoffproject", "GODfather2", "prpl-aim");
    add_account("kerckhoffproject@hotmail.com", "GODfather2", "prpl-msn");
